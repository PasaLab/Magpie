...
public class GeneticSearch_Parallel {
    private static final Logger LOG = LoggerFactory.getLogger(GeneticSearch_Parallel.class);
    ...
    private ArrayList<String> jobTimes = new ArrayList<>();  //记录作业执行时间变化
    private ArrayList<Double> jobScore = new ArrayList<>();  //记录种群性能评分变化
    private ArrayList<Double> totalScore = new ArrayList<>();  //记录个体性能评分变化

    private String submitPattern = Config.getString("job.submit.pattern");

    public GeneticSearch_Parallel(Map<String, String[]> paras){
        this.paras = paras;
    ...
    private int getUsed(Map<String, String> config){
        Double parallel = getDouble(config.get("parallelism.default"));
        Double solt = getDouble(config.get("taskmanager.numberOfTaskSlots"));
        return (int) (parallel/solt)+1;
    }
    ...
    private double[] calculateFitness( ) {
        ...
        for(int i = 0; i < ChrNum; i++){
            int maxYarnResource = 20;
            String str = ipop[i];
            double fitness = 0d;
            List<Map<String, String>> multiConfig = new ArrayList<>();
            //记忆化剪枝
            ...
            if(submitPattern.equals("yarn-cluster")) {
                //根据集群的Yarn资源，计算最多能同时提交多少个作业
                while (maxYarnResource - getUsed(newConfig) > 0 && i < ChrNum) {
                    multiConfig.add(newConfig);
                    iterateCount++;
                    i++;
                    str = ipop[i];
                    newConfig = strToConfig(str);
                    //记忆化剪枝
                    if (allResult.containsKey(str)) {
                        populationFitness[i] = allResult.get(str);
                        LOG.info(String.format("参数配置%s已运行过,适应度计算结果%s", str, allResult.get(str)));
            ...
            try {
                ...
                //把参数加入Flink提交命令中，同时运行多个作业
                FlinkJob job = jobSubmit.executeJobMulti(multiConfig);
