...
public class GeneticSearch_predict {
    private static final Logger LOG = LoggerFactory.getLogger(GeneticSearch_predict.class);
   ...
   // public Double bestScore = 0d;
    ...
    public GeneticSearch_predict(Map<String, String[]> paras){
        this.paras = paras;
    ...
    private String[] primaryPop() {
        ...
        LGBMPredict pd = new LGBMPredict();
        for (int i = 0; i < ChrNum; i++) {
            String temp = configToStr(pd.getParameter());
            while(!rule(temp)){
                temp = configToStr(pd.getParameter());
            }
            ipop[i] = temp;
        }
        return ipop;
